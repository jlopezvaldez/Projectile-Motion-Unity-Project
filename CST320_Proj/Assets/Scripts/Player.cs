...
public class Player : MonoBehaviour
{
    ...
    private bool currentlyOnTurret = false;
...
yVelocity = Mathf.Max(yVelocity - GravityPerSecond * Time.deltaTime, -maxGravity);
    ...
    void Jumping()
    {
        ...
        if (jumpCounter >= 5) {changeColors();};
    }

    void changeColors()
    {
        //GameObject.Find("Capsule").GetComponent<Renderer>().material.color = new Color(color_i/255, 64/255, 64/255, 255/255);
        // GameObject.Find("Capsule").GetComponent<Renderer>().material.SetColor()
        ...
        // Debug.Log("color_i = " + color_i);
        // //print("color_i = " + color_i);

        // if (color_i >= 255) { color_i = 0; };
        GameObject.Find("Capsule").GetComponent<Renderer>().material.color = lerpedColor;
        lerpedColor = Color.Lerp(Color.red, Color.blue, Mathf.PingPong(Time.time, 1));
    }
    //Unity Events:
    
    GameObject getNearestGameObj(string tag) {
        GameObject[] allTurrets = GameObject.FindGameObjectsWithTag(tag);

        float distance = Mathf.Infinity;
        GameObject closest = null;

        foreach (GameObject t in allTurrets) {
            // Vector3 diff = t.transform.position - gameObject.transform.position;
            // float currDistance = diff.sqrMagnitude;
            float currDistance = Vector3.Distance(t.transform.position, gameObject.transform.position);
    ...
    void enterTurret() {
        // Get turret obj that is closest to player
        nearestTurret = getNearestGameObj("Turret");

        // Activate its turretHandler script and activate turret cam
        nearestTurret.GetComponent<turretHandler>().enabled = true;
        Transform turretCam = nearestTurret.transform.Find("Camera");
        turretCam.GetComponent<Camera>().enabled = true;
        
        // Deactivate player camera
        Transform playerCam = gameObject.transform.Find("PlayerCamera");
        playerCam.GetComponent<Camera>().enabled = false;

        // Activate angle viewer script
        GameObject.Find("Canvas").transform.Find("UI_Turret").gameObject.SetActive(true);
        nearestTurret.GetComponent<TurretUI>().enabled = true;

        // Activate arc renderer
        nearestTurret.GetComponent<LineRenderer>().enabled = true;

        // Mark player as on turret
        currentlyOnTurret = true;
    }
    void exitTurret()
    {
        // Get turret obj that is closest to player
        //GameObject nearestTurret = getNearestGameObj("Turret");

        // Deactivate its turretHandler script and deactivate turret cam
        nearestTurret.GetComponent<turretHandler>().enabled = false;
        Transform turretCam = nearestTurret.transform.Find("Camera");
        turretCam.GetComponent<Camera>().enabled = false;

        // Activate player camera
        Transform playerCam = gameObject.transform.Find("PlayerCamera");
        playerCam.GetComponent<Camera>().enabled = true;

        // Deactivate angle viewer script
        GameObject.Find("Canvas").transform.Find("UI_Turret").gameObject.SetActive(false);
        nearestTurret.GetComponent<TurretUI>().enabled = false;

        // Deactivate arc renderer
        nearestTurret.GetComponent<LineRenderer>().enabled = false;
        
        // Mark player as no longer on turret
        currentlyOnTurret = false;
    }

    void mountTurret() {
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (!currentlyOnTurret)
            {
                GameObject nearestTurret = getNearestGameObj("Turret");
                //Vector3 diff = nearestTurret.transform.position - gameObject.transform.position;
                //float currDistance = diff.sqrMagnitude;
                
                float currDistance = Vector3.Distance(nearestTurret.transform.position, gameObject.transform.position);
                if (currDistance < 3)
                {
                    enterTurret();
                }
